...
//);
const ffmpegPath = process.cwd() + "\\ext\\ffmpeg-static\\ffmpeg.exe";
  ...
  return {
    ...
    proxy({ url, verbose, ...options }) {
      ...
      function handler(ws) {
        ...
        if (verbose) {
          const cCon = Inbound[url].total();
          console.log(`[rtsp-relay] WebSocket connection \x1b[33m${cCon}\x1b[0m to ${url}`);
        }
        /** @param {Buffer} chunk */
        ...
        ws.on('close', () => {
          ...
          if (verbose) {
            console.log(`[rtsp-relay] WebSocket to ${url} Disconnected \x1b[33m${c}\x1b[0m left`);
          }
          Inbound[url].kill(c);
